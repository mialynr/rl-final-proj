...
class MLPPolicySL(BasePolicy, nn.Module, metaclass=abc.ABCMeta):
    ...
    def update(self, observations, actions):
        ...
        loss = torch.zeros((1,), device=observations.get_device(), requires_grad=True)
        if True:
            mean = self.mean_net(observations)
            std = torch.exp(self.logstd)
            actions_distribution = torch.distributions.normal.Normal(mean, std)
            log_prob = actions_distribution.log_prob(actions)
            loss = loss - log_prob.sum()
        else:
            for i in range(batch_size):
                mean = self.mean_net(observations[i])
                std = torch.exp(self.logstd)
                actions_distribution = torch.distributions.normal.Normal(mean, std)
                loss = loss - actions_distribution.log_prob(actions[i]).sum()
        loss.backward()
        self.optimizer.step()
        return {
